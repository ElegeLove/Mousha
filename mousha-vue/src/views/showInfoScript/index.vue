...
export default {
    data() {
        return {
            navbarIndex: "BasicInfoInfo",
            navbar: [
                { text: "剧本基础信息", name: "BasicInfoInfo" },
                { text: "流程设定", name: "ProcessSettingInfo" },
                { text: "人物剧本", name: "CharacterPlayInfo" },
                { text: "线索设置", name: "ClueSettingInfo" },
                { text: "地图设置", name: "mapInfo" },
                { text: "问题库", name: "QuestionBankInfo" },
                { text: "结局与真相", name: "EndTruthInfo" }
            ],
            searchContent: "",
            keyWordList: [],
            courseURL: "",
    ...
    created() {
        ...
        // } else {
        //     this.navbarIndex = "BasicInfoInfo";
        // }
        // this.getScriptInfo(id);
        let that = this;
        // localStorage.setItem('selRouter','BasicInfoInfo')
        this.navbar.some(item =>{
            if(localStorage.getItem("selRouter") == item.name){
                that.isGetItems = true;
                return true;
            }else{
                that.isGetItems = false;
            }
        })
        console.log(this.isGetItems)
        if(this.isGetItems){
            this.navbarIndex = localStorage.getItem("selRouter");
            this.$router.push({
                path: '/showInfoScript/'+this.navbarIndex+'?id='+id
                // query: {
                //     type: i
                // }
            });
            ...
            this.navbarIndex = "BasicInfoInfo";
            this.$router.push({
                path: '/showInfoScript/BasicInfoInfo?id='+id
                // query: {
                //     type: i
                // }
            });
        ...
        getScriptInfo(id) {
            ...
            this.$apiAxiox(data).then(res => {
                ...
                res.data.script_role_list.some(item => {
                    ...
                    if (item.script_role_list.length > 0) {
                        item.script_role_list.some(itemChild => {
                            let obj1 = {
                                childMode: itemChild.script_node_name, //选择的流程节点数据的键值
                ...
                res.data.script_role_clue_list.some(item=>{
                    ...
                    // if (item.no_script_role_list.length > 0) {
                    //     item.no_script_role_list.some(itemShow => {
                    //         person2.push(itemShow.script_role_name);
                    //     });
                    // }
                    let obj = {
                        addressName: item.script_role_name, //人物线索名称
                        // showRole: person2, //选择的不可见角色数据的键|[0,1]
                        clueVal: person1,  //线索
                        status: true
                    };
                    person.push(obj);
                })
                that.allData.clueSettingVal = person.concat(arr3);
                // 地图设置
                let arr6 = [];
                ...
                res.data.script_question_list.some((item, index) => {
                    ...
                    let obj = {
                        nodeVal: item.script_node_name, //问题节点的数据的键
                        nameVal:
                            "问题" +
                            (index + 1) +
                            "（" +
