<template>
    <div id="question">
        ...
        <!-- 新增问题 -->
        <el-dialog :visible.sync="isAddProblem" width="30%">
        ...
        >
            <div class="popBox clueSetpop">
                <div class="scoringRules_top">
                    ...
                    <div class="tabMain_top_item_inp">{{ score }}</div>
                    <!-- <input
                        v-model="score"
                        class="tabMain_top_item_inp"
                        type="text"
                ...
                <div>
                    ...
                    <div class="tabMain_top_item_inp">
                        <span v-for="(item, index) in backScore" :key="index"
                            >{{ item }}<span>，</span></span
                        >
                    </div>
                    <!-- <el-select
                    ...
                    <div class="tabMain_top_item_inp">{{ negativeScore }}</div>
                    <!-- <input
...
export default {
    ...
    methods: {
        ...
        sureScore() {
            if (this.score == "") {
                Message.error("请输入得分");
            } else if (this.backScore.length < 1) {
                Message.error("请选择反向得分角色");
            } else if (this.negativeScore == "") {
                Message.error("请输入反向得分");
            } else {
                this.problemData[
                    this.scoreIndex
        ...
        // 确认不可见
        sureShow() {
            if (this.procedureNode.length < 1) {
        ...
        // 下一步
        clickNext() {
            localStorage.setItem('selRouter','QuestionBankUp')
            this.$router.push({
                path: "/updataScript/QuestionBankUp",
                query: {
